...
impl KeyProviderService for KeyProvider {
    ...
    async fn un_wrap_key(
        ...
        debug!("UnWrapping key for kid: {}", annotation.kid);
        ...
        let kbs_url_with_addr = if kbs_addr.is_empty() {
            ...
            format!("{}://{}", kbs_url.scheme(), kbs_addr)
                .parse()
                .map_err(|_| {
        ...
        let kek = get_kek(&kbs_url_with_addr, &kbs_path).await.map_err(|e| {
            error!("Failed to get KEK from KBS: {:?}", e);
            Status::internal(format!("Failed to get KEK from KBS: {:?}", e))
        })?;
        ...
        let optsdata = crate::enc_mods::crypto::decrypt(&wrapped_data, &kek, &iv, &wrap_type)
            .map_err(|e| {
                ...
                Status::internal(format!("Failed to decrypt key: {:?}", e))
            })?;
        ...
        debug!("UnWrapKey API successfully unwrapped key");
